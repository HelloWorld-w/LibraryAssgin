...
//管理员删除座位
if(@$_GET['action']=='del'){
	mysqli_query($_conn,"DELETE FROM ag_adseat WHERE st_stnumber=".$_GET['st_stnumber']);
	if(mysqli_affected_rows($_conn)==1){
			//跳转
			_alert_url_null("恭喜你，删除座位成功！","three_floor.php");
		}else{
			//跳转
			_alert_back_url("删除座位失败！","three_floor.php");
		}	
}
//管理员删除指定的某一些座位
	...
	<div class="page-container">
		...
		<div class="page-content">
			...
			<ol class="breadcrumb bread">
				<li><a href="index.php">控制面板</a>&nbsp;/&nbsp;三楼</li>
			</ol>
			<!-- end content header -->
			...
			<div class="content-body">
				...
				<table class="content-table">
					...
					//分页
					//查询ag_adseat数据库
					$_result=mysqli_query($_conn,"SELECT * FROM ag_adseat WHERE st_stnumber>3000 AND st_stnumber<4000");
					//数据库中一共有多少位子
					...
					if(isset($_GET['curpage'])){
					 ...
					 // echo $start;
					 // exit();
					$limit_data_page=mysqli_query($_conn,"SELECT st_stnumber,st_number FROM ag_adseat WHERE st_stnumber>3000 AND st_stnumber<4000 order by st_stnumber limit " .$start.",66");
					//表格中的每列显示的条数
					$cols_seat=11;
					...
					//把表格中每列的值显示出来
					for($i=1;$i<=$cols;$i++){
						$limit_seat=mysqli_query($_conn,"SELECT st_stnumber,st_number,st_id FROM ag_adseat WHERE st_stnumber>3000 AND st_stnumber<4000 order by st_stnumber limit ".(11*($i-1)+$start).",11");
					?>
						<td>
