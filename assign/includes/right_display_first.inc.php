...
if(@$_POST['action']=='seat_select'){
	...
	if(@mysqli_fetch_array(mysqli_query($_conn,"SELECT st_stnumber FROM ag_seat WHERE st_user='{$_SESSION['user']}' and st_used=0"))){
		_alert_back_url('不得重复占位','first_floor.php?');
	}else{
		_alert_url_null('占位成功！','first_floor.php?');
	}
	//将占位的信息插入数据库表中
...
if(@$_POST['action']=='addtime'){
	...
	if($_addtime=mysqli_fetch_array(mysqli_query($_conn,"SELECT st_stnumber,st_time,st_sumtime FROM ag_seat WHERE st_used=0 AND st_user='{$_SESSION['user']}'"),MYSQLI_ASSOC)){
		...
		if(mysqli_affected_rows($_conn)){
			_alert_url_null('延长占位时间成功！','first_floor.php?');
		}else{
			_alert_back_url('延长占位时间失败！','first_floor.php?');
		}
	}
}
//"我要等待"的这个位置的剩余时间
...
if(@$_POST['action']=='wait'){
	...
	if(mysqli_affected_rows($_conn)==1){
		_alert_url_null('等待成功','first_floor.php');
	}else{
		_alert_back_url('等待失败','first_floor.php');
	}
}
...
<table>
	...
	<?php
	//将数据库中的座位渲染出来
	$_seat=mysqli_query($_conn,"SELECT st_stnumber, st_number FROM ag_adseat WHERE st_stnumber>1000 AND st_stnumber<2000"); 
	// 数据库中有多少位子
	...
	for($i=1;$i<=$rows;$i++){
	$sel_seat=mysqli_query($_conn,"SELECT a1.st_stnumber, a1.st_number,a2.st_user,a2.st_sumtime,a2.st_text,a2.st_wait FROM ag_adseat a1 left join ag_seat a2 on a1.st_stnumber = a2.st_stnumber AND a2.st_used=0 AND a2.st_wait=0 WHERE a1.st_stnumber>1000 AND a1.st_stnumber<2000 limit ".(11*($i-1)).",11");
	$j=1;
	?>
	<tr>
		<td>
	...
	<div class="modal fade" tabindex="-1" id="myModal">
		<div class="modal-dialog sm-lg">
			<form method="post" name="wait" action="first_floor.php">
	...
	<div class="modal fade" tabindex="-1" id="myModal1">
		<div class="modal-dialog sm-lg">
			<div class="modal-content">
				<form method="post" name="seat_select" action="first_floor.php">
	...
	<div class="modal fade" tabindex="-1" id="myModal3">
		<div class="modal-dialog sm-lg">
			<form method="post" name="add_time" action="first_floor.php">
