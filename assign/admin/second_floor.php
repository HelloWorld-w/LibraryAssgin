...
if(@$_GET['action']=='del'){
	...
	if(mysqli_affected_rows($_conn)==1){
			//跳转
			_alert_url_null("恭喜你，删除座位成功！","second_floor.php");
		}else{
			//跳转
			_alert_back_url("删除座位失败！","second_floor.php");
		}	
}
//管理员删除指定的某一些座位
	...
	<div class="page-container">
		...
		<div class="page-content">
			...
			<ol class="breadcrumb bread">
				<li><a href="index.php">控制面板</a>&nbsp;/&nbsp;二楼</li>
			</ol>
			<!-- end content header -->
			...
			<div class="content-body">
				...
				<table class="content-table">
					...
					//分页
					//查询ag_adseat数据库
					$_result=mysqli_query($_conn,"SELECT * FROM ag_adseat
							WHERE st_stnumber >2000 AND st_stnumber<3000");
					//数据库中一共有多少位子
					...
					if(isset($_GET['curpage'])){
					 ...
					 // echo $start;
					 // exit();
					$limit_data_page=mysqli_query($_conn,"SELECT st_stnumber,st_number FROM ag_adseat WHERE st_stnumber >2000 AND st_stnumber<3000 order by st_stnumber limit " .$start.",66");
					//表格中的每列显示的条数
					$cols_seat=11;
					...
					//把表格中每列的值显示出来
					for($i=1;$i<=$cols;$i++){
						$limit_seat=mysqli_query($_conn,"SELECT st_stnumber,st_number,st_id FROM ag_adseat WHERE st_stnumber>2000 AND st_stnumber<3000 order by st_stnumber limit ".(11*($i-1)+$start).",11");
						// echo mysqli_num_rows($limit_seat);
					?>
						<td>
							<table class="content-table2">
								...
								<tr>
									...
									<td><a href="first_floor_update.php?st_id=<?php echo _url_sha1($result['st_id']);?>">修改</a>/<a href="second_floor.php?action=del&&st_stnumber=<?php echo _url_sha1($result['st_stnumber']);?>">删除</a></td>
								</tr>
								<?php }?>
							</table>
						</td>
