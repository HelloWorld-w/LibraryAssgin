...
<table>
	...
	<?php
	//将数据库中的座位渲染出来
	$_seat=mysqli_query($_conn,"SELECT st_stnumber, st_number FROM ag_adseat WHERE st_stnumber>3000 AND st_stnumber<4000"); 
	// 数据库中有多少位子
	...
	for($i=1;$i<=$rows;$i++){
	$sel_seat=mysqli_query($_conn,"SELECT a1.st_stnumber, a1.st_number,a2.st_user,a2.st_sumtime,a2.st_text,a2.st_wait FROM ag_adseat a1 left join ag_seat a2 on a1.st_stnumber = a2.st_stnumber AND a2.st_used=0 AND a2.st_wait=0 where a1.st_stnumber>3000 AND a1.st_stnumber<4000 limit ".(11*($i-1)).",11");
	$j=1;
	?>
	<tr>
		<td>
	...
	<div class="modal fade" tabindex="-1" id="myModal">
		<div class="modal-dialog sm-lg">
			<form method="post" name="wait" action="three_floor.php">
