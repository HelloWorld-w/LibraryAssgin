...
if(@$_POST['action'] == 'register'){
	...
	$_register['email']=_check_email($_POST['email'],6,30);
	$_register['face']=$_POST['face'];
	//查找重复的用户名
	_fetch_array($_conn,"SELECT name FROM ag_username WHERE name='{$_register['usename']}' LIMIT 1",MYSQL_ASSOC,'对不起！用户名已存在');
	//添加新用户
		...
		<div class="modal-dialog">
			<div class="modal-content modal-content-modity">
				...
				<div class="modal-body register-form">
						...
						<input type="text" name="psw" class="input" placeholder="请输入您的用户密码"/><br/>
						<label for="">确 认 密 码：</label>
						<input type="text" name="psw2" class="input" placeholder="请确认您的用户密码"/><br/>
						<label for="">电子邮件：</label>
						<input type="email" name="email" class="input" placeholder="电子邮件"/><br/>
